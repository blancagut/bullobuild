...
export function DealsSection({ products }: DealsSectionProps) {
  ...
  return (
    <section id="deals" className="relative overflow-hidden bg-navy py-20 lg:py-24">
      <div className="absolute inset-y-0 right-0 w-1/3 bg-yellow/[0.035]" aria-hidden="true" />
      <div className="absolute right-0 top-0 h-full w-1 bg-yellow" aria-hidden="true" />

      <Container className="relative z-10">
        <div className="mb-8 flex items-start justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="bg-yellow p-3 text-navy">
              <Zap size={20} />
            </div>
            <SectionHeader
              label="Live deals"
          ...
          <div className="hidden items-center gap-2 text-gray-500 md:flex">
            <Clock size={13} />
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {products.map((product, index) => (
            <div key={product.id} className="space-y-3">
              <div className="flex items-center justify-between border border-yellow/20 bg-yellow/8 px-4 py-3">
        ...
        <div className="mt-8 text-center">
          <Link
            href="/search"
            className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-yellow transition-all hover:gap-4"
          >

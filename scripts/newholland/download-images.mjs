#!/usr/bin/env node
/**
 * New Holland Construction image downloader
 * Reads scripts/newholland/data/newholland-catalog.json
 * Downloads Sitecore Content Hub images to data/product-images/{id}/
 * (Identical platform to Case CE — same CDN)
 */
...
const CATALOG_FILE = join(__dirname, 'data', 'newholland-catalog.json');
...
async function downloadFile(url, dest) {
  await execFileAsync('curl', [
    ...
    '-H', 'Referer: https://construction.newholland.com/',
    '--max-time', '30',
    '-o', dest,
  ]);
}
